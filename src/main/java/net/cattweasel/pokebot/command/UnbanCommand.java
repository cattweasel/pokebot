...
public class UnbanCommand extends AbstractCommand {
	...
	public UnbanCommand() {
		super("/unban", "Entsperrt einen Benutzer dauerhaft");
	}
			...
			if (userId == null) {
				...
				if (!CapabilityManager.hasCapability(resolveUser(context, user), cap)) {
					...
					if (usr == null) {
						...
						auditor.log(Util.otos(user.getId()), AuditAction.UNBAN_USER, usr.getName());
						context.commitTransaction();
						sendMessage(sender, chat, Localizer.localize(resolveUser(context, user), "cmd_unban_success_message"));
					}
				}
			}
